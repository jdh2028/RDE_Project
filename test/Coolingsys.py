...
D_c = 2e-3          # 냉각유로 직경[m]
A_c = (np.pi * D_c**2)/4
N_channels = int(np.floor((np.pi * D_t) / (D_c + 0.001))) # 채널 사이 간격 1mm 가정
print(f"덕트 직경 {D_t*1000:.1f}mm, 채널 직경 {D_c*1000:.1f}mm")
print(f"적용된 냉각 채널 개수: {N_channels} 개")
...
for i, oi in enumerate(x):
    ...
    M_dot_c_per_channel = M_dot_c / N_channels
    U_c_local = M_dot_c_per_channel / (rho_c * A_c)
    ...
    for iter_count in range(max_iter):
        ...
        if abs(T_wg_new - T_wg) < tol:
            converged = True # 수렴 성공!
            T_wg = T_wg_new # 최종 값 업데이트
            break
        
        T_wg = (1-alpha)*T_wg + alpha*T_wg_new
    
    if not converged:
        print(f"경고: x = {oi:.4f} m 위치에서 수렴에 실패했습니다. (반복 횟수: {iter_count+1})")
        # break 
    ...
    T_wc_final = T_c + q_final / h_c

    
   
    heat_Area = np.pi * D_t * dx
    Q_total  = q_final * heat_Area

    delta_T_c = Q_total / (M_dot_c * Cp_c)
    T_c_new = T_c + delta_T_c
...
ax1.set_xlabel('Nozzle Axial Position x [m]', fontsize=18)
...
# 오른쪽 y축: 열유속
ax2 = ax1.twinx()
ax2.plot(x, q_arr, label='Heat Flux q', color='orange', linewidth=3)
ax2.set_ylabel('Heat Flux [W/m²]', fontsize=18)
ax2.tick_params(axis='y')


ax1.legend(loc='upper right', fontsize=16, bbox_to_anchor=(1, 0.75))
ax2.legend(loc='upper right', fontsize=16, bbox_to_anchor=(1, 0.85))  


plt.title('Temperature and Heat Flux Distribution Along 1-D Nozzle', fontsize=20)
plt.tight_layout()
plt.show()
